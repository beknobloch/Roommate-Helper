...
def calculate_totals(ldger, usr):
    ...
    for itm in ldger.item_list:
        ...
        # if user uses the item subtract 1 from the length (this will help calculate what is owed)
        if usr in itm.users_who_use:
            length = len(itm.users_who_use) - 1
        else:
        ...
        for person in itm.users_who_use:
            # if we're at the person who paid, continue
            if person == usr:
                continue
